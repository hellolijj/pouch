...
func TestParseDiskQuota(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "test5", args: args{diskquota: []string{"  "}}, want: nil, wantErr: true},
		{name: "test6", args: args{diskquota: nil}, want: nil, wantErr: true},
		{name: "test7", args: args{diskquota: []string{"foo=foo", "bar=bar"}}, want: map[string]string{"foo": "foo", "bar": "bar"}, wantErr: false},
	}
